...
<div x-data="dashboardManager({{ $initialData }})">
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
        ...
        <div class="bg-white rounded-xl shadow-sm p-5 border-l-4 border-green-500">
            ...
            <p class="text-2xl font-bold">$<span x-text="stats.todayRevenue.toFixed(2)">{{ number_format($todayRevenue, 2) }}</span></p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <tbody>
                        <template x-for="order in recentOrders" :key="order.order_no">
                            <tr class="border-b border-gray-100">
                                ...
                                <td class="py-2">$<span x-text="order.grand_total.toFixed(2)"></span></td>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            ...
            <p class="text-3xl font-bold text-green-600 mb-2">$<span x-text="dailySales.total_sales.toFixed(2)">{{ number_format($dailySales['total_sales'] ?? 0, 2) }}</span></p>
            <div class="space-y-2 text-sm">
                ...
                <div class="flex justify-between"><span class="text-gray-500">Cash</span><span>$<span x-text="dailySales.cash_sales.toFixed(2)">{{ number_format($dailySales['cash_sales'] ?? 0, 2) }}</span></span></div>
                <div class="flex justify-between"><span class="text-gray-500">Card</span><span>$<span x-text="dailySales.card_sales.toFixed(2)">{{ number_format($dailySales['card_sales'] ?? 0, 2) }}</span></span></div>
            </div>
        </div>
    </div>
</div>
